...
void LinkBasedGASolver::mutate(HFT::Chromosome& offspring) {
    // Geometric distribution to determine the distance between bit flips.
...
void LinkBasedGASolver::crossover(HFT::Chromosome& parent1, HFT::Chromosome& parent2) {
    ...
    // Swap segments between parents. Handles bit-ranges within 64-bit blocks for optimisation.
    for (int i = start_block; i <= end_block; ++i) {
        int start_bit = (i == start_block) ? (start_idx % 64) : 0;
        int end_bit = (i == end_block) ? (end_idx % 64) : 63;

        // Create a mask for the specific bit range within the current 64-bit word.
        ...
        // Apply the swapped segments using bitwise masks.
...
HFT::FitnessPair LinkBasedGASolver::get_chromosome_fitness(const HFT::Chromosome& chromosome) {
    SelectionEvaluator evaluator{ m_graph, m_requests, m_max_latency };
    ...
    // If recording is enabled, reconstruct the list of edges used by this chromosome.

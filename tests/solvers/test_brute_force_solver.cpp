...
TEST_F(BruteForceSolverTest, ReturnsTheGloballyOptimalProfit) {
    ...
    graph.add_edge({ .id = 4, .source = 7, .dest = 3, .rate_limit = 6, .latency = 17, .lease_cost = 20 }, false);
    graph.add_edge({ .id = 5, .source = 3, .dest = 7, .rate_limit = 10, .latency = 19, .lease_cost = 15 }, false);
    graph.add_edge({ .id = 6, .source = 8, .dest = 1, .rate_limit = 2, .latency = 26, .lease_cost = 15 }, false);
    graph.add_edge({ .id = 7, .source = 5, .dest = 8, .rate_limit = 3, .latency = 50, .lease_cost = 70 }, false);
    graph.add_edge({ .id = 8, .source = 8, .dest = 4, .rate_limit = 1, .latency = 20, .lease_cost = 30 }, false);
    graph.add_edge({ .id = 9, .source = 4, .dest = 3, .rate_limit = 1, .latency = 20, .lease_cost = 30 }, false);
